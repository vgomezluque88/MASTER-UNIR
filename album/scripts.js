...
// Nivel 1: sacar un 10 en la actividad
// Nivel 2: cómo se hacía hace 5 años (pre SPA y web components)

const pintarId = (id) => {
    // 1. ir a buscar la información de ese id
    const photo = photos.find(photo => photo.id === id)
    // 2. crear un img
    const img = document.createElement("img")
    img.src = photo.url
    img.classList.add("img-responsive")
    // 3. escribirlo en album-preview
    const albumPreview = document.querySelector("#album-preview")
    albumPreview.innerHTML = ""
    albumPreview.append(img)
}

const inicializarAlbum = () => {
    const thumbnailsWrapper = document.querySelector("#album-thumbnails")
    ...
    photos.forEach(photo => {
        ...
        thumbnailsWrapper.append(img)
    })

    thumbnailsWrapper.addEventListener("click", (e) => {
        pintarId(Number(e.target.dataset.id))
    })
